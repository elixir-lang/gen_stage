...
defmodule Flow.Window.SessionTest do
  ...
  describe "single window" do
    ...
    test "trigger keep with large demand" do
      assert Flow.from_enumerable(1..100)
             |> Flow.partition(window: single_window() |> Flow.Window.trigger_every(10), stages: 1, key: fn _ -> 0 end)
             |> Flow.reduce(fn -> 0 end, & &1 + &2)
             |> Flow.emit(:state)
    ...
    test "trigger keep with small demand" do
      assert Flow.from_enumerable(1..100)
             |> Flow.partition(window: single_window() |> Flow.Window.trigger_every(10), stages: 1, max_demand: 5, key: fn _ -> 0 end)
             |> Flow.reduce(fn -> 0 end, & &1 + &2)
             |> Flow.emit(:state)
             |> Enum.to_list() == [55, 210, 465, 820, 1275, 1830, 2485, 3240, 4095, 5050, 5050]
    end
    ...
    test "trigger discard with large demand" do
      assert Flow.from_enumerable(1..100)
             |> Flow.partition(window: single_window() |> Flow.Window.trigger_every(10, :reset), stages: 1, key: fn _ -> 0 end)
             |> Flow.reduce(fn -> 0 end, & &1 + &2)
             |> Flow.emit(:state)
    ...
    test "trigger discard with small demand" do
      assert Flow.from_enumerable(1..100)
             |> Flow.partition(window: single_window() |> Flow.Window.trigger_every(10, :reset), stages: 1, max_demand: 5, key: fn _ -> 0 end)
             |> Flow.reduce(fn -> 0 end, & &1 + &2)
             |> Flow.emit(:state)
             |> Enum.to_list() == [55, 155, 255, 355, 455, 555, 655, 755, 855, 955, 0]
    end
    ...
    test "trigger based on intervals" do
      assert Flow.from_enumerable(Stream.concat(1..10, Stream.timer(:infinity)), max_demand: 5, stages: 2)
             |> Flow.partition(window: single_window() |> Flow.Window.trigger_periodically(100, :millisecond),
                               stages: 1, max_demand: 10, key: fn _ -> 0 end)
             |> Flow.reduce(fn -> 0 end, & &1 + &2)
             |> Flow.map_state(& &1 * 2)
             |> Flow.emit(:state)
             |> Enum.take(1) == [110]
    ...
    test "trigger based on timers" do
      assert Flow.from_enumerable(Stream.concat(1..10, Stream.timer(:infinity)), max_demand: 5, stages: 2)
             |> Flow.partition(stages: 1, max_demand: 10, window: single_window(), key: fn _ -> 0 end)
             |> Flow.reduce(fn ->
  ...
  describe "double ordered windows with single key" do
    ...
    test "triggers per window with large demand" do
      assert Flow.from_enumerable(1..100, stages: 1)
             |> Flow.partition(window: double_ordered_window() |> Flow.Window.trigger_every(12), stages: 1, key: fn _ -> 0 end)
             |> Flow.reduce(fn -> 0 end, & &1 + &2)
             |> Flow.map_state(fn state, _, {:session, session, trigger} -> [{state, session, trigger}] end)
             |> Enum.to_list() == [{78, {0, 1, 50}, {:every, 12}},
                                   {300, {0, 1, 50}, {:every, 12}},
                                   {666, {0, 1, 50}, {:every, 12}},
    ...
    test "reduces per window with small demand" do
      assert Flow.from_enumerable(1..100, stages: 1)
             |> Flow.partition(window: double_ordered_window(), stages: 1, max_demand: 5, min_demand: 0, key: fn _ -> 0 end)
             |> Flow.reduce(fn -> 0 end, & &1 + &2)
             |> Flow.emit(:state)
             |> Enum.to_list() == [1275, 3775]
    end
    ...
    test "triggers per window with small demand" do
      assert Flow.from_enumerable(1..100, stages: 1)
             |> Flow.partition(window: double_ordered_window() |> Flow.Window.trigger_every(12),
                               stages: 1, max_demand: 5, min_demand: 0, key: fn _ -> 0 end)
             |> Flow.reduce(fn -> 0 end, & &1 + &2)
             |> Flow.map_state(fn state, _, {:session, session, trigger} -> [{state, session, trigger}] end)
             |> Enum.to_list() == [{78, {0, 1, 15}, {:every, 12}},
                                   {300, {0, 1, 25}, {:every, 12}},
                                   {666, {0, 1, 40}, {:every, 12}},
                                   {1176, {0, 1, 50}, {:every, 12}},
                                   {1275, {0, 1, 50}, :done},
  ...
  describe "double ordered windows with multiple keys" do
    ...
    test "triggers per window with large demand" do
      assert Flow.from_enumerable(1..100, stages: 1)
             |> Flow.partition(window: double_ordered_window() |> Flow.Window.trigger_every(12), stages: 1, key: &rem(&1, 2))
             |> Flow.reduce(fn -> 0 end, & &1 + &2)
             |> Flow.map_state(fn state, _, {:session, session, trigger} -> [{state, session, trigger}] end)
             |> Enum.sort() == [{144, {1, 1, 49}, {:every, 12}},
                                {156, {0, 2, 50}, {:every, 12}},
                                {576, {1, 1, 49}, {:every, 12}},
                                {600, {0, 2, 50}, {:every, 12}},
                                {625, {1, 1, 49}, :done},
    ...
    test "reduces per window with small demand" do
      assert Flow.from_enumerable(1..100, stages: 1)
             |> Flow.partition(window: double_ordered_window(), stages: 1, max_demand: 5, min_demand: 0, key: &rem(&1, 2))
             |> Flow.reduce(fn -> 0 end, & &1 + &2)
             |> Flow.emit(:state)
             |> Enum.to_list() == [625, 650, 1900, 1875]
    end
    ...
    test "triggers per window with small demand" do
      assert Flow.from_enumerable(1..100, stages: 1)
             |> Flow.partition(window: double_ordered_window() |> Flow.Window.trigger_every(12),
                               stages: 1, max_demand: 5, min_demand: 0, key: &rem(&1, 2))
             |> Flow.reduce(fn -> 0 end, & &1 + &2)
             |> Flow.map_state(fn state, _, {:session, session, trigger} -> [{state, session, trigger}] end)
             |> Enum.sort() == [{144, {1, 1, 25}, {:every, 12}},
                                {156, {0, 2, 24}, {:every, 12}},
                                {576, {1, 1, 49}, {:every, 12}},
                                {600, {0, 2, 50}, {:every, 12}},
                                {625, {1, 1, 49}, :done},
