defmodule GenStage.DemandDispatcherTest do
  ...
  @default_infos %{shuffle_demands_on_first_dispatch: false, demands_seq: :natural}

  defp dispatcher(opts) do
    shuffle_demand = Keyword.get(opts, :shuffle_demands_on_first_dispatch, false)
    {:ok, {[], 0, nil, %{shuffle_demands_on_first_dispatch: ^shuffle_demand, demands_seq: :natural}} = state} = D.init(opts)
    state
  end
  ...
  test "subscribes and cancels" do
    ...
    assert disp == {[{0, pid, ref}], 0, nil, @default_infos}

    {:ok, 0, disp} = D.cancel({pid, ref}, disp)
    assert disp == {[], 0, nil, @default_infos}
  end
  ...
  test "subscribes, asks and cancels" do
    ...
    assert disp == {[{10, pid, ref}], 0, 10, @default_infos}

    {:ok, 0, disp} = D.cancel({pid, ref}, disp)
    assert disp == {[], 10, 10, @default_infos}

    # Subscribe, ask and cancel and leave the same demand
    {:ok, 0, disp} = D.subscribe([], {pid, ref}, disp)
    assert disp == {[{0, pid, ref}], 10, 10, @default_infos}

    {:ok, 0, disp} = D.ask(5, {pid, ref}, disp)
    assert disp == {[{5, pid, ref}], 5, 10, @default_infos}
  ...
  test "subscribes, asks and dispatches" do
    ...
    assert disp == {[{3, pid, ref}], 0, 3, @default_infos}

    {:ok, [], disp} = D.dispatch([:a], 1, disp)
    assert disp == {[{2, pid, ref}], 0, 3, @default_infos}
    assert_received {:"$gen_consumer", {_, ^ref}, [:a]}

    {:ok, 3, disp} = D.ask(3, {pid, ref}, disp)
    assert disp == {[{5, pid, ref}], 0, 3, @default_infos}

    {:ok, [:g, :h], disp} = D.dispatch([:b, :c, :d, :e, :f, :g, :h], 7, disp)
    assert disp == {[{0, pid, ref}], 0, 3, @default_infos}
  ...
  test "subscribes, asks multiple consumers" do
    ...
    assert disp == {[{4, pid, ref1}, {3, pid, ref3}, {2, pid, ref2}], 0, 4, @default_infos}

    {:ok, 2, disp} = D.ask(2, {pid, ref3}, disp)
    assert disp == {[{5, pid, ref3}, {4, pid, ref1}, {2, pid, ref2}], 0, 4, @default_infos}

    {:ok, 4, disp} = D.ask(4, {pid, ref2}, disp)
    assert disp == {[{6, pid, ref2}, {5, pid, ref3}, {4, pid, ref1}], 0, 4, @default_infos}
  end
  ...
  test "subscribes, asks and dispatches to multiple consumers" do
    ...
    {:ok, 2, disp} = D.ask(2, {pid, ref2}, disp)
    assert disp == {[{3, pid, ref1}, {2, pid, ref2}], 0, 3, @default_infos}
    ...
    assert disp == {[{0, pid, ref1}, {0, pid, ref2}], 0, 3, @default_infos}
    refute_received {:"$gen_consumer", {_, _}, _}

    # Two batches with left over
    {:ok, 3, disp} = D.ask(3, {pid, ref1}, disp)
    {:ok, 3, disp} = D.ask(3, {pid, ref2}, disp)
    assert disp == {[{3, pid, ref1}, {3, pid, ref2}], 0, 3, @default_infos}

    {:ok, [], disp} = D.dispatch([:a, :b], 2, disp)
    assert disp == {[{3, pid, ref2}, {1, pid, ref1}], 0, 3, @default_infos}
    assert_received {:"$gen_consumer", {_, ^ref1}, [:a, :b]}

    {:ok, [], disp} = D.dispatch([:c, :d], 2, disp)
    assert disp == {[{1, pid, ref1}, {1, pid, ref2}], 0, 3, @default_infos}
  ...
  test "subscribes, asks and dispatches to multiple consumers with shuffled demands" do
    ...
    {:ok, 3, disp} = D.ask(3, {pid, ref1}, disp)
    {:ok, 2, disp} = D.ask(2, {pid, ref2}, disp)
    assert disp == {[{3, pid, ref1}, {2, pid, ref2}], 0, 3, %{shuffle_demands_on_first_dispatch: true, demands_seq: :natural}}

    # demands should be shuffled after first dispatch
    {:ok, [], disp} = D.dispatch([:a, :b, :c, :d, :e], 5, disp)

    {[{0, pid, ref1_actual}, {0, pid, ref2_actual}], 0, 3, %{shuffle_demands_on_first_dispatch: true, demands_seq: :shuffled}} = disp
